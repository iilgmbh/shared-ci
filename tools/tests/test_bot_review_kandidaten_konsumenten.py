...
def _pr(nr, **o):
    b = dict(number=nr, isDraft=False, author={"login": "achimdehnert"}, mergeStateStatus="CLEAN",
             reviewDecision=None, reviews=[], files=[{"path": "src/x.py"}],
             statusCheckRollup=[{"name": "ci", "conclusion": "SUCCESS", "startedAt": "2026-09-16T10:00:00Z"}])
    b.update(o); return b
...
def test_should_keep_deploy_definition_and_migrations_human(monkeypatch):
    monkeypatch.setenv("AUCH_CLEAN", "1")
    prs = [_pr(1, files=[{"path": ".github/workflows/deploy.yml"}]),
